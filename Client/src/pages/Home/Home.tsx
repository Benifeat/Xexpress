          ...
          {loading ? (
            <>
              {Array.from({ length: 8 }).map((_, index) => (
                <Skeleton
                  key={index}
                  className="w-[23%] min-h-[50%] h-72 aspect-square p-0 rounded-md"
                />
              ))}
            </>
          ) : cards.length > 0 ? (
              ...
              {cards.map((card: any) => (
                ...
                >
                  ...
                  <Popover placement="bottom" showArrow={true}>
                    <PopoverTrigger>
                      <Button className="absolute rounded-full hover:-translate-y-1 bg-red-600 font-bold text-xl bottom-2 right-2"  onClick={(e) => {
                              e.stopPropagation();
                              if (
